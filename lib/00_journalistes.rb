...
def menu(journalistes)
  puts "Bonjour on va analyser les Twitter handles des journalistes"
  puts "Choisis une question à laquelle tu veux une reponse en tapant le chiffre, eg: 1"
  puts "\n"
...
def answer(journalistes,user_choice)
  if user_choice == 1
    	puts "Il y a #{journalistes.size} journalistes" #Combien y a-t-il de journalistes dans ce array ?
  elsif user_choice == 2
      puts journalistes.count{|h| h.match?(/\d/)} #Combien d'handle contiennent un numéro ?
  elsif user_choice == 3
  		puts journalistes.count{|h| h.include?("aude") || h.include?("Aude")} #Combien d'handle contiennent les 4 lettres du prénom "Aude" à la suite (sans prendre en compte les majuscules) ?
  elsif user_choice == 4
  		puts journalistes.count{|h| h =~ /^@[[:upper:]]/} #Combien commencent par une majuscule (première lettre juste après le @) ?
  elsif user_choice == 5
  		puts journalistes.count{|h| h.match?(/[[:upper:]]/)} #Combien contiennent au moins une majuscule ?
  elsif user_choice == 6
  		puts journalistes.count{|h| h.include?("_")} #Combien y a-t-il de underscore _ dans tous les pseudos confondus ?
  elsif user_choice == 7
  		puts journalistes.sort(&:casecmp) #Trie la liste de handle par ordre alphabétique.
  elsif user_choice == 8
  		puts (journalistes.sort_by(&:length)[0..50])  #Quels sont les 50 handles les plus courts de ce array ?
  elsif user_choice == 9
  		puts journalistes.index("@epenser") #Quelle est la position dans l'array de la personne @epenser ?
  else
    puts "Choisis entre 1 et 9!"
  end
end

def perform(journalistes)
  user_choice = menu(journalistes)
  answer(journalistes,user_choice)
end
...
puts journalistes.count{|handle| handle[1].match?( /[[:upper:]]/ )} 
...
N'hésite pas à ajouter un peu de pep’s à ton programme en écrivant des phrases, en ajoutant des blagues, voir en faisant un menu 
(pour les plus chauds) où l'utilisateur peut choisir, dans une liste, la question à laquelle le programme va répondre.
=end